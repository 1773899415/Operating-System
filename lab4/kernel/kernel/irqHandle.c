...
void syscallAdd(struct StackFrame *sf);
void syscallSub(struct StackFrame *sf);
...
void syscallReadStdIn(struct StackFrame *sf) {
	...
	if (dev[STD_IN].value == 0) {
		...
		pcb[current].sleepTime = 2147483647;
		pcb[current].state = STATE_BLOCKED;
		asm volatile("int $0x20");
		int seg_data = sf->ds, size = sf->ebx, count = 0;
		char *s = (char*)sf->edx, character = 0;
		asm volatile("movw %0, %%es"::"m"(seg_data));
		while (bufferHead != bufferTail) {
			...
			if (character != 0) {
				asm volatile("movb %0, %%es:(%1)"::"r"(character),"r"(s + count));
				++count;
			}
		}
		asm volatile("movb $0x00, %%es:(%0)"::"r"(s + count));
		pcb[current].regs.eax = count;
	}

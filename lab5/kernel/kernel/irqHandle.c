...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if (((sf->edx >> 3) & 1) == 0 && destInode.type == DIRECTORY_TYPE) {
			pcb[current].regs.eax = -1;
			return;
		}
		if (((sf->edx >> 3) & 1) == 1 && destInode.type == REGULAR_TYPE) {
			pcb[current].regs.eax = -1;
			return;
		}
		int flag = 2;
...
void syscallWrite(struct StackFrame *sf) {
	...
	if (file_idx >= 0 && file_idx < MAX_FILE_NUM) {
		if (file[file_idx].state == 1)
			syscallWriteFile(sf);
		else
			pcb[current].regs.eax = -1;
	}
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	if (size <= sBlock.blockSize - remainder) {
		...
		while(i < size) {
			...
			++i;
			if (!((remainder + i) % sBlock.blockSize)) {
...
void syscallReadFile(struct StackFrame *sf) {
	...
	int file_idx = sf->ecx - MAX_DEV_NUM, left = inode.size - file[file_idx].offset;
	if (left < size)
		size = left;
	readBlock(&sBlock, &inode, quotient + j, buffer);
	if (size <= sBlock.blockSize - remainder) {
...
void syscallLseek(struct StackFrame *sf) {
	...
	diskRead(&inode, sizeof(Inode), 1, file[sf->ecx - MAX_DEV_NUM].inodeOffset);
	int file_idx = i - MAX_DEV_NUM;
	switch(sf->ebx) { // whence
		case SEEK_SET:
			file[file_idx].offset = offset;
			pcb[current].regs.eax = file[file_idx].offset;
			break;
		case SEEK_CUR:
			file[file_idx].offset += offset;
			pcb[current].regs.eax = file[file_idx].offset;
			break;
		case SEEK_END:
			file[file_idx].offset = inode.size + offset;
			pcb[current].regs.eax = file[file_idx].offset;
			break;
		default:
			break;
...
void syscallStat(struct StackFrame *sf) {
	...
	char *str1 = (char*)sf->ecx + baseAddr, *str2 = (char*)sf->edx + baseAddr;
	Inode destInode;
	int destInodeOffset = 0;
	ret = readInode(&sBlock, gDesc, &destInode, &destInodeOffset, str1);
	if (ret == 0) {
		for (int i = 0; i < 12; ++i)
			str2[i] = *(char*)((char*)(&destInode) + i);
		pcb[current].regs.eax = 0;
	}
	else
		pcb[current].regs.eax = -1;

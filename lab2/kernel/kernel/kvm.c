...
void loadUMain(void) {
	...
	int phoff = 0;
	int offset = 0;
	unsigned int uMainEntry = 0;
	unsigned int startpos = 0x200000; 
	
	for (i = 0; i < 200; i++)
		readSect((void*)(startpos + i*512), 201+i);

	uMainEntry = ((struct ELFHeader *)startpos)->entry;
	phoff = ((struct ELFHeader *)startpos)->phoff;
	offset = ((struct ProgramHeader *)(startpos + phoff))->off;

	for (i = 0; i < 200 * 512; i++)
		*(unsigned char *)(startpos + i) = *(unsigned char *)(startpos + i + offset);

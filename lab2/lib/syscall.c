...
char getChar(){ // 对应SYS_READ STD_IN
	char c = (char)syscall(SYS_READ, STD_IN, 0, 0, 0, 0);
	while (c == 0)
		c = (char)syscall(SYS_READ, STD_IN, 0, 0, 0, 0);
	return c;
}

void getStr(char *str, int size){ // 对应SYS_READ STD_STR
	uint32_t flag = syscall(SYS_READ, STD_STR, (uint32_t)str, (uint32_t)size, 0, 0);
	while (flag == 0)
		flag = syscall(SYS_READ, STD_STR, (uint32_t)str, (uint32_t)size, 0, 0);
}
	...
	while(format[i] != 0){
		if (format[i] == '%') {
			...
			if (format[i] == 'd')
				...
				count = hex2Str(*((int*)(paraList)), buffer, MAX_BUFFER_SIZE, count);
			else if (format[i] == 's')
				count = str2Str((char*)(*((uint32_t*)paraList)), buffer, MAX_BUFFER_SIZE, count);
			else if (format[i] == 'c')
